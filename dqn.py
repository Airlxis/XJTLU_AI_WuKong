...
writer = SummaryWriter(log_dir="D:\\Black-Myth-Wukong-AI-main\\Black-Myth-Wukong-AI-main\\runs")
...
def dqn_learning(env,
    ...
    model_resnet_boss.load_state_dict(torch.load("D:\\Black-Myth-Wukong-AI-main\\Black-Myth-Wukong-AI-main\\dataset\\boss_model.pkl", map_location=torch.device('cpu')))
    ...
    # 初始自身模型
     ...
     #model_resnet_malo.load_state_dict(torch.load('D:/dqn_wukong/RL-ARPG-Agent-1/malo_model.pkl'))
    ...
    # load checkpoint
    if checkpoint != 0:
        checkpoint_path = "D:\\Black-Myth-Wukong-AI-main\\Black-Myth-Wukong-AI-main\\models\\wukong_0904_1_0.pth"
    ...
    for t in itertools.count(start=checkpoint):
        ...
        self_power_window = (1785,1034,1843,1092) # 棍势点
        ...
        self_endurance_window = (214,1061,531,1069) # 耐力条
        ...
        ding_shen_window = (1651,837,1701,891)
        ...
        if indices_boss.item() in [4, 9]:  # 锄地
            action = torch.tensor([selected_num])
        elif indices_boss.item() in [1, 2, 5]:  # 锄地起飞
            action = torch.tensor([selected_num])
        ...
        # 攻击但没有耐力了
        if action.item() not in [3, 1] and self_endurance < 30 and self_endurance != 0:
            action = torch.tensor([5])

        # 可识破
        if indices_boss.item() == 1 and self_power > 50:
